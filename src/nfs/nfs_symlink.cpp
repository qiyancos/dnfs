...
int nfs3_symlink(nfs_arg_t *arg, struct svc_req *req, nfs_res_t *res){
    ...
    if (arg->arg_symlink3.where.dir.data.data_len == 0) {
        rc = NFS_REQ_ERROR;
    ...
    get_file_handle(arg->arg_symlink3.where.dir);

    LOG(MODULE_NAME, D_INFO,
        "The value of the arg_symlink obtained dir handle is '%s', and the length is '%d'",
        arg->arg_symlink3.where.dir.data.data_val,
        arg->arg_symlink3.where.dir.data.data_len);
    out:
