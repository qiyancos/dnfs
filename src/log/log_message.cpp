...
LogMessage::LogMessage(const string &module_name,
    ...
    /*建立临时缓存存储数据*/
    char *buffer_message = (char *) gsh_calloc(INIT_MESSAGE_BUFFER, sizeof(char));
    ...
    /*格式化字符串*/
    int message_len = vsnprintf(buffer_message, INIT_MESSAGE_BUFFER, format, args);
    ...
    /*判断数据大小*/
    if (message_len >= INIT_MESSAGE_BUFFER) {
        /*设置新的指针*/
