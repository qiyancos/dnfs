...
int nfs3_getattr(nfs_arg_t *arg, struct svc_req *req, nfs_res_t *res)
{
    ...
    LOG(MODULE_NAME, D_INFO, "The value of the nfs_getattr obtained file handle is '%s', and the length is '%d'",
    ...
    {
        rc=NFS_REQ_ERROR;
        LOG(MODULE_NAME, L_ERROR, "Interface nfs_getattr failed to obtain '%s' attributes",
            arg->arg_getattr3.object.data.data_val);
        goto out;
    }
