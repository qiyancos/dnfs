...
main(int argc, char **argv)
{
    ...
    /*cpu软限制是内核强加给相应资源的限制值，硬限制是软限制的最大值，得到软硬资源限制*/
	...
	/* Initialize logging.创建启动日志 */
	if (!foreground) xlog_stderr(0);
	xlog_open(progname);

    /*忽略信号，即使没有意义，代码执行仍将继续*/
	...
	/* 子进程结束时, 父进程会收到这个信号这个信号只能在linux和sysv使用，BSD不能用 */
	sigaction(SIGCHLD, &sa, NULL);

	/*在rpc启动前关闭自身大于2的进程 Daemons should close all extra filehandles ... *before* RPC init. */
	if (!foreground)
		closeall(3);

    /*测试是否"nfsd" filesystem 开启*/
